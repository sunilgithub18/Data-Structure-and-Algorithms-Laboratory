...
INPUT: keys-k, weights-w, successful probabilities-p, unsuccessful probabilities -q
OUTPUT: OBST

Author:Prof. Nilesh D. Mali.
Date: 21.05.2023
...
class OBST
{
	...
	public:
		...
		void get_data()
		{
			...
			cin>>n;
			cout<<"\n Enter the data as…\n";
			//Elements from which OBST is to be built
			for(i=1;i<=n;i++)
			{
				cout<<"\n a["<<i<<"]=>";
				cin>>a[i];
			...
			//Probabilities with which we search for an element
			for(i=1;i<=n;i++)
			{
				cout<<"\n p["<<i<<"]=>";
				cin>>p[i];
			...
			//Probabilities that an element is not found
			for(i=0;i<=n;i++)
			{
				cout<<"\n q["<<i<<"]=>";
				cin>>q[i];
